...
function Write-Log {
    ...
    # Check log size every 50 writes to avoid a file-stat on each call
...
function Write-SiemEvent {
    ...
    } catch { Write-Host "SIEM write error: $_" }
}
...
function Get-Whitelist {
    ...
    if (Test-Path $wlFile) {
        try {
            $script:_wlCache = (Get-Content $wlFile -Raw | ConvertFrom-Json) -as [string[]]
            return $script:_wlCache
        }
        catch { $script:_wlCache = $DefaultWhitelist; return $script:_wlCache }
    }
    $script:_wlCache = $DefaultWhitelist
    return $script:_wlCache
}
...
function Send-DiscordAlert {
    ...
    } catch { Write-Log "Discord alert error: $_" }
}
...
function Send-EmailAlert {
    ...
    } catch { Write-Log "Email alert error: $_" }
}
...
function Get-VirusTotalReport {
    ...
    } catch { Write-Log "VirusTotal API error: $_"; return $null }
}
...
function Get-ProcDetails {
    ...
    try {
        ...
        $ownerInfo = $null
        try { $ownerInfo = Invoke-CimMethod -InputObject $proc -MethodName GetOwner } catch { $ownerInfo = $null }
        return [PSCustomObject]@{
            Name        = $proc.Name
            PID         = $proc.ProcessId
    ...
    } catch { Write-Log "Get-ProcDetails error for PID ${Pid}: $_"; return $null }
}
...
function Get-NetworkConnsForPid {
    ...
    } catch { Write-Log "Get-NetworkConnsForPid error for PID ${Pid}: $_"; return $null }
}
...
function Get-FileSignatureStatus {
    ...
    } catch { Write-Log "Signature check error for ${FilePath}: $_"; return "sig-check-failed" }
}
...
function Get-FileHashSafe {
    ...
    } catch { Write-Log "File hash error for ${FilePath}: $_" }
    return $null
}
...
function Backup-FileToStore {
    ...
    } catch { Write-Log "Backup error for ${FilePath}: $_" }
}
