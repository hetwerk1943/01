...
function Send-CollectorAlert {
    ...
    $collectorUrl = $env:COLLECTOR_ENDPOINT
    $jwt          = $env:COLLECTOR_JWT
    if ([string]::IsNullOrWhiteSpace($collectorUrl) -or [string]::IsNullOrWhiteSpace($jwt)) { return }
    $json = $Payload | ConvertTo-Json -Compress
    try {
        # mTLS + JWT auth enforced
