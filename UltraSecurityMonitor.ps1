...
function Write-SecureLog {
    ...
    $logFile = "$PSScriptRoot\logs\security.log"
    New-Item -Path "$PSScriptRoot\logs" -ItemType Directory -Force | Out-Null

    $bytes   = [System.Text.Encoding]::UTF8.GetBytes($Message)
    # LocalMachine scope: any user on this machine can decrypt. Use CurrentUser scope for stronger per-user isolation.
    $encData = [System.Security.Cryptography.ProtectedData]::Protect(
        $bytes, $null, [System.Security.Cryptography.DataProtectionScope]::LocalMachine)
    Add-Content -Path $logFile -Value ([Convert]::ToBase64String($encData))
}
