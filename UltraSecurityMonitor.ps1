...
# Set these in your shell profile or CI/CD secrets before running the script:
...
# E-mail alerty – enabled when all three SMTP_* env vars are present
$SmtpServer = $env:SMTP_SERVER
$SmtpFrom   = $env:SMTP_FROM
$SmtpTo     = $env:SMTP_TO
$SmtpUseSsl = $true
$SmtpPort   = if ($env:SMTP_PORT) { [int]$env:SMTP_PORT } else { 587 }
$EmailAlerts = (-not [string]::IsNullOrWhiteSpace($SmtpServer)) -and
               (-not [string]::IsNullOrWhiteSpace($SmtpFrom))   -and
               (-not [string]::IsNullOrWhiteSpace($SmtpTo))
