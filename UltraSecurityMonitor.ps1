...
# Enforce TLS 1.2 for all outbound web requests
...
$SmtpUseSsl       = $true
$SmtpPort         = 587
# Opcjonalne: dane logowania SMTP (SecureString lub $null aby pominąć uwierzytelnianie)
$SmtpCredential   = $null   # np. Get-Credential lub New-Object PSCredential("user", (ConvertTo-SecureString "pass" -AsPlainText -Force))
...
function Sanitize-LogEntry {
    param([string]$msg)
    # Strip newlines, tabs and control characters to prevent log injection;
    # keep only printable ASCII and common extended Latin (U+00A0-U+024F)
    return ($msg -replace '[\r\n\t]', ' ') -replace '[^\x20-\x7E\u00A0-\u024F]', ''
}
...
function Write-Log {
    ...
    $entry = "$ts`t$(Sanitize-LogEntry $msg)"
...
function Send-DiscordAlert {
    ...
    # Validate the webhook URL is an HTTPS Discord URL to prevent SSRF
    if ($DiscordWebhookUrl -notmatch '^https://discord(?:app)?\.com/api/webhooks/\d+/[A-Za-z0-9_-]+$') {
        Write-Log "Send-DiscordAlert: invalid webhook URL, skipping"
        return
    }
...
function Send-EmailAlert {
    ...
    try {
        ...
        if ($null -ne $SmtpCredential) { $params['Credential'] = $SmtpCredential }
...
function Backup-FileToStore {
    ...
    try {
        ...
        # Prevent path traversal: ensure destination resolves inside BackupFolder
        $resolvedDest   = [System.IO.Path]::GetFullPath($dest)
        $resolvedBackup = $BackupFolder.TrimEnd([System.IO.Path]::DirectorySeparatorChar) + [System.IO.Path]::DirectorySeparatorChar
        $resolvedBackup = [System.IO.Path]::GetFullPath($resolvedBackup)
        if (-not $resolvedDest.StartsWith($resolvedBackup)) { return }
        Copy-Item -Path $FilePath -Destination $resolvedDest -Force -ErrorAction SilentlyContinue
    } catch {}
}
