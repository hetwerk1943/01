...
# --------- KONFIGURACJA ---------
# Hardening defaults – remediation is DISABLED by default (dry-run mode)
$EnableDryRun      = $true
$EnableRemediation = $false

# Import secure helpers module
Import-Module (Join-Path $PSScriptRoot 'SecureHelpers.psm1') -Force -ErrorAction SilentlyContinue

# Propagate to module globals consumed by SecureHelpers
$Global:EnableDryRun      = $EnableDryRun
$Global:EnableRemediation = $EnableRemediation
...
$procAction = {
    try {
        ...
        if (Test-ProcessSuspicious -ProcName $details.Name -FilePath $path) {
            # VirusTotal lookup (cached, no raw args in cache)
            $vtResult = $null
            if (-not [string]::IsNullOrWhiteSpace($hash)) {
                $vtResult = Get-VirusTotalReportCached -Hash $hash
            }

            # Build sanitized summary – raw command line is never stored
            $summary = Build-AlertSummary -ProcessName $details.Name -Hash $hash `
                           -HostName $env:COMPUTERNAME -Owner $details.Owner `
                           -SigStatus $sigStatus -CmdLine $details.CommandLine

            # Enrich with VT results if available
            if ($null -ne $vtResult) {
                $summary['vt_malicious']  = $vtResult.Malicious
                $summary['vt_suspicious'] = $vtResult.Suspicious
            }

            # Log sanitized summary only
            $logLine = "⚠️ SUSPECT PROCESS | Name=$($summary['name']) PID=$pid " +
                       "Owner=$($summary['owner']) Sig=$($summary['sig']) " +
                       "SHA256=$($summary['hash']) Cmd=$($summary['cmd'])"
            if ($null -ne $vtResult) {
                $logLine += " VT:Malicious=$($summary['vt_malicious']) VT:Suspicious=$($summary['vt_suspicious'])"
            }
            Write-Log $logLine
            Add-Content -Path $ReportPath -Value ("`n`n$(Get-Date -Format o)`n$logLine")

            # SIEM event uses sanitized command line (never raw)
            Write-SiemEvent -EventType "SuspiciousProcess" -Severity "High" -Data @{
                name        = $summary['name']
                pid         = $pid
                hash        = $summary['hash']
                sig         = $summary['sig']
                commandLine = $summary['cmd']
                owner       = $summary['owner']
                host        = $summary['host']
                timestamp   = $summary['timestamp']
            }

            # Send redacted alerts
            ...
            # NOTE: Any future remediation (Stop-Process, Remove-Item, Move-Item, etc.)
            # MUST be wrapped in Execute-RemediationSafe to respect $EnableDryRun /
            # $EnableRemediation globals, e.g.:
            #   Execute-RemediationSafe -Reason "Kill $($details.Name)" -ActionScriptBlock {
            #       Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue
            #   }
            #
            # NOTE: Consider encrypting $LogPath and $ReportPath with EFS and restricting
            # their ACLs to the service account only to protect sensitive event data at rest.
        }
