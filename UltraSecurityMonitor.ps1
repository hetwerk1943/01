...
function Get-VT-Cache {
    ...
    try {
        $all = Get-Content $CacheFile -Raw | ConvertFrom-Json
        if ($null -ne $all -and $all.PSObject.Properties[$Hash]) { return $all.$Hash }
    } catch {}
    return $null
}

function Update-VT-Cache {
    param([string]$Hash, $Result, [string]$CacheFile)
    try {
        $cache = [ordered]@{}
        if (Test-Path $CacheFile) {
            $existing = Get-Content $CacheFile -Raw | ConvertFrom-Json
            if ($null -ne $existing) {
                $existing.PSObject.Properties | ForEach-Object { $cache[$_.Name] = $_.Value }
            }
        }
        $cache[$Hash] = $Result
        $tmp = $CacheFile + ".tmp"
        $cache | ConvertTo-Json | Out-File $tmp -Encoding UTF8
...
function Get-VirusTotalReport {
    ...
    $cached = Get-VT-Cache -Hash $Hash -CacheFile $VTCacheFile
    ...
    try {
        ...
        Update-VT-Cache -Hash $Hash -Result $result -CacheFile $VTCacheFile
        return $result
    } catch { return $null }
}
