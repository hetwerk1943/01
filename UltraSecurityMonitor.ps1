...
# --------- KONFIGURACJA ---------
# Hardening defaults: remediation is disabled and dry-run is enabled.
# Set $EnableRemediation = $true and $EnableDryRun = $false only in a controlled test VM.
$EnableDryRun       = $true
...
$procAction = {
    try {
        ...
        if (Test-ProcessSuspicious -ProcName $details.Name -FilePath $path) {
            # Build redacted summary – raw CommandLine is never sent outside this host
            $vtResult = $null
            ...
            # NOTE: Recommend restricting ACL or enabling EFS on security-report.txt and security.log
            #       to prevent unauthorised local access. Do not forward raw logs outside the Collector.
            Add-Content -Path $ReportPath -Value ("`n`n$(Get-Date -Format o)`n" + ($summary | ConvertTo-Json -Depth 3))
            Write-SiemEvent -EventType "SuspiciousProcess" -Severity "High" -Data @{
                ...
                hash        = $summary.hash
                sig         = $summary.sig
                commandLine = $summary.cmd   # sanitized – no raw args
                owner       = $summary.owner
            }
            ...
            # NOTE: Any future Stop-Process / Remove-Item / Move-Item remediation actions
            # MUST be wrapped in Execute-RemediationSafe { ... } -Reason "..." to respect
            # the EnableDryRun / EnableRemediation flags and prevent accidental destruction.
        }
