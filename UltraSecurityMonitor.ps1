...
# Counter for throttled log-rotation stat check
[long]$script:_logWriteCount = 0
# Cache for whitelist to avoid repeated disk reads
...
function Write-Log {
    ...
    if ($script:_logWriteCount % 50 -eq 0) {
        try {
...
function Get-Whitelist {
    ...
    if (Test-Path $wlFile) {
        try { $script:_wlCache = (Get-Content $wlFile -Raw | ConvertFrom-Json) -as [string[]]; return $script:_wlCache }
