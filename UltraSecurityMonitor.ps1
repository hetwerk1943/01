...
function Get-VT-Cache {
    ...
    try {
        ...
        $entry = $cache.PSObject.Properties[$Hash]
...
function Update-VT-Cache {
    ...
    try {
        ...
        $cache | ConvertTo-Json -Compress | Set-Content $CacheFile -Force
    } catch {}
}
...
function Send-CollectorAlert {
    ...
    Send-DiscordAlert $json
    Send-EmailAlert -Subject "Security Alert: $($Payload.ProcessName)" -Body $json
    Write-SiemEvent -EventType "ProcessAlert" -Severity "Medium" -Data $Payload
}
