...
function Send-CollectorAlert {
    ...
    if ([string]::IsNullOrWhiteSpace($env:COLLECTOR_JWT)) { return }
    $CollectorUrl = "https://collector.local/api/telemetry"
    try {
        $json = $Payload | ConvertTo-Json -Compress
        Invoke-RestMethod -Uri $CollectorUrl -Method Post -Body $json `
            -Headers @{ "Authorization" = "Bearer $env:COLLECTOR_JWT" } `
...
function Register-FolderMonitor {
    ...
    try {
        ...
        $action = {
            ...
            Send-CollectorAlert @{ event_type = "FileChange"; severity = "Low"; path = $path; change = $changeType.ToString(); host = $env:COMPUTERNAME; user = $env:USERNAME; timestamp = (Get-Date).ToString("o") }
        }
...
$procAction = {
    try {
        ...
        if (Test-ProcessSuspicious -ProcName $details.Name -FilePath $path) {
            ...
            Send-CollectorAlert @{ event_type = "SuspiciousProcess"; severity = "High"; name = $details.Name; pid = $pid; path = $path; hash = $hash; sig = $sigStatus; commandLine = $details.CommandLine; owner = $details.Owner; host = $env:COMPUTERNAME; user = $env:USERNAME; timestamp = (Get-Date).ToString("o") }
        }
