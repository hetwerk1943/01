...
# --------- SECURE HARDENING ---------
# Dry-run enabled and remediation disabled by default for safety.
# To enable remediation in a controlled environment:
#   1. Review SecureHelpers.psm1 and SECURITY-HARDENING.md.
#   2. Set $EnableDryRun = $false and $EnableRemediation = $true ONLY after review.
$EnableDryRun      = $true
$EnableRemediation = $false
Import-Module (Join-Path $PSScriptRoot "SecureHelpers.psm1") -Force
# Propagate flags to module-level globals used by Execute-RemediationSafe
...
$procAction = {
    try {
        ...
        if (Test-ProcessSuspicious -ProcName $details.Name -FilePath $path) {
            # Build redacted summary – raw command line stays local only
            $summary = Build-AlertSummary -Name $details.Name -Owner $details.Owner `
                           -Sig $sigStatus -Hash $hash -CommandLine $details.CommandLine

            $vtNote = ""
            # VirusTotal lookup (cached)
            if (-not [string]::IsNullOrWhiteSpace($hash)) {
                $vtResult = Get-VirusTotalReportCached -Hash $hash
                if ($null -ne $vtResult) {
                    $vtNote = "VirusTotal: Malicious=$($vtResult.Malicious) Suspicious=$($vtResult.Suspicious)"
                }
            }

            # Local log may retain full detail; restrict file ACLs / use EFS for sensitive logs.
            $localMsg = "⚠️ SUSPECT PROCESS`nName: $($details.Name)`nPID: $pid`nOwner: $($details.Owner)`nPath: $path`nSig: $sigStatus`nSHA256: $hash`nCmd: $($details.CommandLine)"
            if ($null -ne $network -and $null -ne $network.TCP) {
                $localMsg += "`nTCP:`n" + ($network.TCP |
                    Select-Object LocalAddress, LocalPort, RemoteAddress, RemotePort, State |
                    Out-String)
            }
            if ($vtNote) { $localMsg += "`n$vtNote" }

            Write-Log $localMsg
            # Local report may contain full details; ensure $ReportPath is ACL-restricted or EFS-encrypted.
            Add-Content -Path $ReportPath -Value ("`n`n$(Get-Date -Format o)`n$localMsg")

            # SIEM event: commandLine field uses sanitized value to avoid exfiltration
            Write-SiemEvent -EventType "SuspiciousProcess" -Severity "High" -Data @{
                ...
                hash        = $hash
                sig         = $sigStatus
                commandLine = $summary.cmd   # sanitized – no raw args in SIEM output
                owner       = $details.Owner
            }

            # External alerts use the redacted summary only
