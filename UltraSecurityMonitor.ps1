...
# --------- KONFIGURACJA ---------
# Remediation is disabled by default; set $EnableRemediation = $true (with care)
# after reviewing Execute-RemediationSafe in SecureHelpers.psm1.
$EnableDryRun      = $true
$EnableRemediation = $false

Import-Module (Join-Path $PSScriptRoot 'SecureHelpers.psm1') -Force
Set-Variable -Name 'Global:EnableDryRun'      -Value $EnableDryRun      -Scope Global
Set-Variable -Name 'Global:EnableRemediation' -Value $EnableRemediation -Scope Global
...
$procAction = {
    try {
        ...
        if (Test-ProcessSuspicious -ProcName $details.Name -FilePath $path) {
            # Build sanitized summary – raw command line and network data are never logged
            ...
            # Log only sanitized JSON (no raw cmd, no raw network data)
            # Consider enabling EFS on $LogPath and restricting its ACL for additional protection.
            Write-Log ("Suspicious: " + ($summary | ConvertTo-Json -Compress))
            Add-Content -Path $ReportPath -Value ("`n`n$(Get-Date -Format o)`n" + ($summary | ConvertTo-Json -Compress))

            # SIEM export uses sanitized commandLine only
            ...
            # Any future remediation (Stop-Process, Remove-Item, Move-Item, etc.) MUST be
            # wrapped with Execute-RemediationSafe so it is gated by $EnableDryRun /
            # $EnableRemediation. Example:
            #   Execute-RemediationSafe -Reason "Kill $($summary.name)" -ActionScriptBlock {
            #       Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue
            #   }
            # Move-ToEncryptedQuarantine can then be called inside the scriptblock
            # when Execute-RemediationSafe returns $true.
        }
