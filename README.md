...
```bash
git clone https://github.com/hetwerk1943/Ultra-Security-Monitor.git
cd Ultra-Security-Monitor
```

Następnie utwórz folder docelowy i skopiuj pliki:

```powershell
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\Documents\SecurityMonitor"
Copy-Item UltraSecurityMonitor.ps1 "$env:USERPROFILE\Documents\SecurityMonitor\"
Copy-Item dashboard.html "$env:USERPROFILE\Documents\SecurityMonitor\"
```
