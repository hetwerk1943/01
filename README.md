...
- **Pamięć podręczna wyników** (`vt-cache.json`) – każdy hash jest sprawdzany tylko raz; wynik jest zachowany w pamięci i na dysku, co eliminuje zbędne zapytania API i respektuje limit 4 req/min darmowego planu.
...
| `vt-cache.json` | Persystentna pamięć podręczna wyników VirusTotal (SHA-256 → wynik) |
