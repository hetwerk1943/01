...
| Plik / Folder | Opis |
|---------------|------|
| `src/UltraSecurityMonitor.ps1` | Główny skrypt PowerShell – silnik monitoringu (Hardened Edition) |
| `src/Write-SecureLog.ps1` | Moduł bezpiecznego logowania do folderu `logs\` |
| `src/VT-Cache.ps1` | Moduł cache'owania wyników VirusTotal API |
| `src/Send-CollectorAlert.ps1` | Moduł centralnego wysyłania alertów (Discord, e-mail, CollectorAPI) |
| `src/CollectorAPI.ps1` | Moduł integracji ze zdalnym endpointem kolekcjonera zdarzeń |
| `logs/` | Folder logów z restrykcyjnym ACL (tylko SYSTEM i Administratorzy) |
| `Setup-Logs.ps1` | Jednorazowy skrypt konfiguracji folderu `logs\` z restrykcyjnym ACL |
| `dashboard.html` | Dashboard HTML/JS do wizualizacji logów i alertów |
| `README.md` | Dokumentacja projektu |

> **Uwaga:** `UltraSecurityMonitor.ps1` w katalogu głównym pozostaje jako wersja standalone (bez modułów).
...
## 🔹 Hardened Setup (ULTRAMASTER APOCALYPSE)

### Krok 1 – Konfiguracja folderu logów

Uruchom **raz** jako Administrator, aby utworzyć folder `logs\` z restrykcyjnymi uprawnieniami:

```powershell
.\Setup-Logs.ps1
```

Skrypt wykona następujące polecenia:

```powershell
icacls .\logs /inheritance:r
icacls .\logs /grant:r "SYSTEM:(OI)(CI)F" "Administrators:(OI)(CI)F"
```

Po tej operacji tylko konto **SYSTEM** i członkowie grupy **Administrators** mają dostęp do folderu `logs\`.

### Krok 2 – Uruchomienie wersji modularnej

```powershell
cd "<ścieżka-do-repozytorium>"
.\src\UltraSecurityMonitor.ps1
```

Skrypt automatycznie załaduje moduły z tego samego katalogu `src\`:
`Write-SecureLog.ps1`, `VT-Cache.ps1`, `Send-CollectorAlert.ps1`, `CollectorAPI.ps1`.
