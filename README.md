...
| `package.json` | Konfiguracja Node.js – serwer deweloperski (`http-server`) |
...
### Dashboard przez serwer lokalny (Node.js `http-server`)

Jeśli chcesz otworzyć dashboard przez lokalny serwer HTTP (np. aby uniknąć ograniczeń CORS przy ładowaniu plików), możesz użyć `http-server`:
...
2. Uruchom serwer:
   ...
   npm start
   ```
3. Otwórz w przeglądarce: [http://localhost:8080/dashboard.html](http://localhost:8080/dashboard.html)
